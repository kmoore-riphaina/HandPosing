...
namespace PoseAuthoring
{
    public class HandPoseRecorder : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(recordKey) || Recording)
            {
                if (Input.GetKeyDown(recordKey) && Recording)
                {
                    ...
                    if (ghosts.Count > 0)
                    {
                        GenerateAnimations(ghosts, (float)(ghosts.Count / timer.Elapsed.TotalSeconds));
                        ghosts = new List<HandGhost>();
                    }
                }
                else
                {
        ...
        private void GenerateAnimations(List<HandGhost> ghosts, float framerate)
        {
            ...
            foreach (var g in ghosts)
            {
                //var parent = g.transform.FindChildRecursive("hands:b_l_hand");
                //if (parent == null)
                //{
                //    parent = g.transform.FindChildRecursive("hands:b_r_hand");
                //}
